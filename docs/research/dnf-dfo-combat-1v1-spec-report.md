...
가장 중요한 구현 포인트는 전투 로직을 **60Hz 논리 프레임** 기준으로 두고, 스킬 전체가 아니라 **모션 구간 단위**로 공격속도/캐스팅속도/이동속도 영향을 분리하는 것입니다. 공식 패치 노트는 특정 스킬이 공격속도의 영향을 받도록 바뀌거나, 시전 모션이 캐스팅속도의 영향을 받도록 바뀌거나, 심지어 공격속도의 **고정 비율**만 적용되도록 바뀌는 사례를 반복해서 보여 줍니다. 커뮤니티 60fps 측정은 여기에 프레임 단위의 구간(브레이크포인트)이 실제로 존재함을 보강합니다. citeturn29search1turn29search10turn29search7turn40search12turn14view0

커맨드 시스템은 **방향키 선입력**과 **발동키 수리(受理) 시점**을 분리해서 구현해야 합니다. 공개 자료에 따르면 방향키는 다른 스킬 동작 중에도 미리 입력할 수 있지만, 마지막 발동키는 현재 상태가 허용될 때 눌러야 합니다. 또한 2026년 한/중 서버는 중복 커맨드 규칙을 “기본 쿨타임이 가장 긴 스킬 우선, 같으면 습득 레벨이 높은 스킬 우선”으로 정리했고, 최대 4개 스킬을 묶는 스킬 체인 시스템을 추가했습니다. citeturn19search0turn19search1turn20view0turn42search5

레이드/방 구조는 **서버 권한형 인스턴스 상태 머신**으로 보는 것이 가장 타당합니다. 공식 가이드는 보스 HP/무력화 수치의 유지, 전장 오브젝트의 지역 이동과 재생성, 페이즈 전환 시 강화/재생성, 전투 진입/처치/시간 초과/후퇴/전멸에 따른 캐스팅 정지·취소·즉시 실행을 모두 명시하고 있습니다. 이런 규칙은 클라이언트 단독 상태로는 일관되게 재현하기 어렵습니다. citeturn35view0turn35view1turn35view2turn35view3